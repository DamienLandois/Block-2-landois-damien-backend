...
describe('UserService', () => {
  ...
  describe('getUsers', () => {
    ...
    it('returns an array of users', async () => {
      const mockUsers = [
        { id: 'a1b2c3', email: 'damien.landois@test.com', firstname: 'Damien', name: 'Landois', phoneNumber: '0625468547' },
      ];
  ...
  describe('createUser', () => {
    ...
    it('throws ConflictException when email already in use (P2002)', async () => {
      ...
      await expect(
        service.createUser({ email: 'utilisateur.existant@test.com', password: 'MotDePasseValide123!' } as any),
    ...
    it('rethrows unknown errors', async () => {
      ...
      await expect(
        service.createUser({ email: 'test.test@test.com', password: 'AutreMotDePasse2024!' } as any),
      ).rejects.toBe(err);
    });
  });
  ...
  describe('updateUser', () => {
    ...
    it('hashes password when provided', async () => {
      prisma.user.update.mockResolvedValue({ id: 'a1b2c3', email: 'damien.landois@test.com' });

      await service.updateUser('a1b2c3', { password: 'NouveauMotDePasse2024!' });
    ...
    it('rethrows unknown errors', async () => {
      ...
      await expect(service.updateUser('a1b2c3', { name: 'TestNom' })).rejects.toBe(err);
    });
  });
  ...
  describe('deleteUser', () => {
    ...
    it('deletes user (void)', async () => {
      ...
      expect(prisma.user.delete).toHaveBeenCalledWith({ where: { id: 'a1b2c3' } });
    });
